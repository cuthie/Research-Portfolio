...
for iter_lp=1:iter
    ...
    % Sorted n_change
    n_ch_sort=sort(norm_n_change(iter_lp,:));
    
    % T-k Change Point
    n_change_l(iter_lp)=n_ch_sort(l);
...
% 
% % Probability parameter for T-l
% p_delay_l=zeros(n_delay,1);
% 
% % Loop for delay counter
% for r=1:n_delay
%     
%     % Computing delay threshold
%     delay(r)=1+2000*(r-1)/(n_delay);
...
%     for iter_lp=1:iter
%         
%         % Condition for counting for probability parameter for T-l
%         if(epsilon*n_change_l(iter_lp)>=delay(r))
%             
%             % Incrementing probabilty parameter
%             p_delay_l(r)=p_delay_l(r)+1;
%             
%         end
%         
%     end
...
% hold on
% grid on
% % plot(delay, (-epsilon*log((1/iter)*p_delay_min)./delay),'m*-');
% % hold on
...
%     xbin(s)=(s-1)*(m/100);
%
%     % Inner term initialization
%     y=0;
%
%     % Inner term for the distribution
%     for j=1:l
%
%         y=y+(exp(-xbin(s))/(n-j+1));
%
%     end
%
%     % Theoretical pdf
...
% [cden,xden]=ksdensity(n_change_l,'Function','cdf');
% 
% % Sum term
...
%     y=0;
%     
%     % Inner Loop for summation
%     for i=l:n
%         
%         % The Binomial Summation term
%         y=y+((nchoosek(n,i)*(normcdf(xden(s),0,1)^i))*((1-normcdf(xden(s),0,1))^(n-i)));
%         
%     end
%     
%     % Assignment
...
% Mean simulated detection delay
m_ch_l=mean(n_change_l);

%% Theoretical Mean of the Distribution of the Detection Delay
c=factorial(n)*(2^(1.5-l))/((pi^0.5)*factorial(l-1)*factorial(n-l));
...
for u=0:1:n-l
    ...
    for p=1:2:l+u-1
        ...
        t=t+(nchoosek(l+u-1,p)*(pi^(-p/2))*(2^p)*gamma((p+2)/2)*Lauricella_A(p));
