...
for iter_lp=1:iter
    ...
    %     % T-min Change Point
    %     n_change_min(iter_lp)=min(n_change(iter_lp,:));
    
    % T-max Change Point
    n_change_max(iter_lp)=max(norm_n_change(iter_lp,:));
...
[cden,xden]=ksdensity(n_change_max,'Function','pdf');
% mu_sim=mean(ksdensity(n_change_max));
% pd=fitdist(n_change_max,'Normal');
% 
% m_sim=mean(pd);
...
%         %fm(s)=(3*lam*exp(-lam*xbin(s)))-(6*lam*exp(-2*lam*xbin(s)))+(3*lam*exp(-3*lam*xbin(s)));
%     %     %fm(s)=exp(-(xbin(s)+exp(-xbin(s))));
%     %fm(s)=n*lam*exp(-lam*xden(s))*((1-(exp(-lam*xden(s))))^(n-1));
%     %     %fm(s)=n*lam*exp(-lam*xbin(s))*(exp(-n*exp(-lam*xbin(s))));
%     %     fm(s)=lam*exp(-exp((log(n))-(lam*xbin(s))))*exp((log(n))-(lam*xbin(s)));
%     
%     % Theoretical cdf
%     cm(s)=(0.5*(1+erf((xden(s)-m_t)/(sqrt(2)*sig_t))))^n;
%     
% end

% cm=normcdf(xden,0,1).^n;
...
for p=1:2:(n-1)
    ...
    % Summmation variable
    s=s+(nchoosek(n-1,p)*(pi^(-p/2))*(2^p)*gamma((p+2)/2)*Lauricella_A(p));
